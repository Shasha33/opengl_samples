...
obj_model_t::obj_model_t(char const * filename)
{
   ...
   for (auto const & shape : shapes)
   {
      for (int i = 0; i < shape.mesh.indices.size(); ++i)
      {
         ...
         // const auto tex_x = attrib.texcoords[2 * shape.mesh.indices[i].texcoord_index];
         // const auto tex_y = attrib.texcoords[2 * shape.mesh.indices[i].texcoord_index + 1];
         // vertices.push_back({tex_x, tex_y, 0});
      }
   }
